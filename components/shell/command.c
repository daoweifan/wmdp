...
static cmd_t *__name2cmd(const char *name)
{
	cmd_t **entry, **end;
	if(name != NULL) {
		entry = (cmd_t **)SECTION_BEGIN(shell_cmd);
		end = (cmd_t **)SECTION_END(shell_cmd);
		while(entry < end) {
...
int cmd_queue_update(struct cmd_queue_s *cq)
{
	...
	list_for_each_safe(pos, bkup, &cq -> cmd_list) {
		clst = list_entry(pos, struct cmd_list_s, list);
...
int cmd_queue_exec(struct cmd_queue_s *cq, const char *cl)
{
	...
	n = strlen(cl);
	clst = malloc(sizeof(struct cmd_list_s) + n + 1);
	ASSERT(cq != NULL);
